...
from edge_lake.generic import process_status, params, utils_data, process_log
...
def protocol_exec(status, command: str, protocol_callbacks: ProtocolCallbacks,
                ...
                if j_instance.is_job_active() and j_instance.get_unique_job_id() == status.get_unique_job_id():
                    ...
                    if j_handle.is_subset():
                        ...
                        if error_list:
                            ...
                            err_msg = f"Partial results: {len(error_list)} nodes failed"
    ...
    # Case D: Stream file (e.g., image, PDF)
        ...
        err_msg = f"File streaming not yet implemented for {protocol_callbacks.get_protocol_name()}"
        status.add_error(err_msg)
